<?php
//echo $fecha;
$fecha = date('Y-m-d', strtotime(str_replace(array('%20', '-'), array(' ', '+'), $fecha)));
//id, suc, fecha, hora_inicio, hora_fin, asunto, estatus, detalle, supervisor, gerente, alta, modi
echo form_open('bitacora/submit_agregar', array('id' => 'form_agregar'));
...
<table style="font-size: large; width: 100%;">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
            <?php
            echo form_dropdown('suc', $suc, '', 'id="suc"');
            ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
            <?php
            echo form_dropdown('asunto', $asuntos, '', 'id="asunto"');
            ?>
            </td>
        </tr>
   ...
   $(function() {
        ...
        */
        $('#form_agregar').submit(function(event){
            ...
            var url = "<?php echo site_url();?>/bitacora/submit_agregar";
            ...
            if(titulo.length >=3 && suc > 0 && asunto > 0 && fecha.length == 10 && hora_inicio.length == 5 && hora_fin.length == 5)
            {
                if(confirm("Seguro que deseas agregar este evento ala bitacora ?")){
                    $.post( url, variables, function(data) {
                        if(data > 0)
                        {
                            window.location = "<?php echo site_url();?>/bitacora";
                        }else{
                            alert('No se pudo agregar el evento a la bitacora');
                        }
